...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _build_model(self, config):
        ...
        trust_remote_code = config.model.get("trust_remote_code", False)
        # Keep FlashAttention as the upstream default, but allow launchers to
        # select SDPA on hosts whose NVIDIA driver cannot execute the bundled
        # FlashAttention cubin.  The actor path already exposes the same
        # choice through model.override_config.attn_implementation.
    ...
    def _compute_entropy_safe(self, logits, chunk_size=4096):
        ...
        # Response logits are commonly produced by slicing a larger sequence
        # tensor and can therefore be non-contiguous. ``view`` rejects that
        # valid layout; ``reshape`` preserves the intended flattening and only
        # copies when the strides require it.
