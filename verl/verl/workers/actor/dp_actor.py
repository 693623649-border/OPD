...
from verl.utils.opd import build_topk_distillation_tensors, validate_topk_replay_tensors
...
class DataParallelPPOActor(BasePPOActor):
    ...
    def compute_distillation_reward(self, data: DataProto) -> DataProto:
        ...
        support_weight_normalization = data.meta_info.get("support_weight_normalization", "author")
        micro_batch_size = data.meta_info["micro_batch_size"]
        temperature = data.meta_info["temperature"]
        use_dynamic_bsz = data.meta_info["use_dynamic_bsz"]
        ...
        res_tensors = build_topk_distillation_tensors(
            strategy=strategy,
            ...
            student_on_teacher_log_probs=S_on_T,
            student_in_teacher_mask=overlap_mask,
            teacher_in_student_mask=teacher_in_student_mask,
            support_weight_normalization=support_weight_normalization,
        )
    ...
    def update_policy(self, data: DataProto):
        ...
        if data.batch["advantages"].dim() == 3:
            # PPO replay must compare old/current log-probs for the same token
            # IDs.  Fail closed instead of falling back to student IDs when a
            # teacher/union support forgot its matching old log-probs.
            validate_topk_replay_tensors(data.batch)
