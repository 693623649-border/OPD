                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        ...
                        if self.use_rm and "rm_scores" not in batch.batch.keys():
                            ...
                            support_weight_normalization = self.config.actor_rollout_ref.rollout.get(
                                "support_weight_normalization", "author"
                            )
                            ...
                            batch.meta_info["support_weight_normalization"] = support_weight_normalization
                    ...
                    if need_recomputation:
                        ...
                        if entropys is not None:
                            ...
                            if "teacher_entropy" in batch.batch.keys():
                                ...
                                # Paper Eq. (8) averages the absolute entropy
                                # difference at each visited state.  This is not
                                # equivalent to subtracting two scalar means.
                                entropy_gap = eq8_absolute_entropy_gap(
                                    entropys,
                                    teacher_entropy,
                                    response_masks,
                                )
                                if entropy_gap is not None:
                                    metrics["opd/entropy_gap_schema_version"] = 1
                                    metrics["opd/abs_entropy_gap"] = entropy_gap.detach().item()

                                position_log_freq = int(
                                    self.config.trainer.get("position_entropy_log_freq", 0)
                                )
                                position_start_step = int(
                                    self.config.trainer.get("position_entropy_start_step", 0)
                                )
                                if (
                                    position_log_freq > 0
                                    and self.global_steps >= position_start_step
                                    and self.global_steps % position_log_freq == 0
                                ):
                                    position_bin_size = int(
                                        self.config.trainer.get("position_entropy_bin_size", 256)
                                    )
                                    student_bins, token_counts = binned_masked_mean(
                                        entropys,
                                        response_masks,
                                        position_bin_size,
                                    )
                                    teacher_bins, teacher_counts = binned_masked_mean(
                                        teacher_entropy,
                                        response_masks,
                                        position_bin_size,
                                    )
                                    if not torch.equal(token_counts, teacher_counts):
                                        raise RuntimeError(
                                            "student and teacher position-entropy masks produced different counts"
                                        )
                                    metrics["opd/position_entropy_schema_version"] = 1
                                    metrics["position-entropy/bin_size"] = position_bin_size
                                    metrics["position-entropy/student_mean_by_bin"] = (
                                        student_bins.detach().cpu().tolist()
                                    )
                                    metrics["position-entropy/teacher_mean_by_bin"] = (
                                        teacher_bins.detach().cpu().tolist()
                                    )
                                    metrics["position-entropy/token_count_by_bin"] = (
                        ...
                        if "overlap_mask" in batch.batch.keys() and "advantages" in batch.batch.keys():
                            try:
                                ...
                                if top_k > 0 and advantages.dim() == 3:
                                    ...
                                    union_strategy = strategy in {"union", "union-intersection"}
                                    is_union = union_strategy and (adv_k == 2 * top_k)

                                    # Paper Eq. (7) is a diagnostic distribution on
                                    # the overlap itself.  Keep it separate from the
                                    # strategy-specific training reward: in the
                                    # non-overlap ablation that reward is zero on the
                                    # intersection by construction.
                                    if student_log_probs is not None and teacher_on_stu_log_probs is not None:
                                        eq7_value = eq7_intersection_advantage(
                                            student_log_probs,
                                            teacher_on_stu_log_probs,
                                            overlap_mask,
                                            response_mask,
                                        )
                                        if eq7_value is not None:
                                            metrics["opd/metric_schema_version"] = 2
                                            metrics["val-topk/adv_intersection"] = eq7_value.item()

                                        diagnostic_chunk_size = 1024
                                        for diagnostic_start in range(0, max_len, diagnostic_chunk_size):
                                            diagnostic_end = min(diagnostic_start + diagnostic_chunk_size, max_len)
                                            diagnostic_value = eq7_intersection_advantage(
                                                student_log_probs[:, diagnostic_start:diagnostic_end],
                                                teacher_on_stu_log_probs[:, diagnostic_start:diagnostic_end],
                                                overlap_mask[:, diagnostic_start:diagnostic_end],
                                                response_mask[:, diagnostic_start:diagnostic_end],
                                            )
                                            if diagnostic_value is not None:
                                                diagnostic_key = f"{diagnostic_start}_{diagnostic_end}"
                                                metrics[f"val-topk/adv_intersection_chunk_{diagnostic_key}"] = (
                                                    diagnostic_value.item()
                                                )

                                    # Figure 19 selects v*=argmax |adv(v)| on the
                                    # overlap of the student-side Top-k support,
                                    # then averages p_student(v*)-p_teacher(v*).
                                    # It cannot be reconstructed by subtracting
                                    # the separately averaged max/min metrics below.
                                    if (
                                        strategy in {"only_stu", "intersection", "union", "union-intersection"}
                                        and student_log_probs is not None
                                        and teacher_on_stu_log_probs is not None
                                    ):
                                        student_side_advantages = student_side_topk_advantages(
                                            advantages,
                                            top_k,
                                            union_support=union_strategy,
                                        )
                                        figure19_value = prob_diff_at_max_abs_advantage(
                                            student_side_advantages,
                                            student_log_probs,
                                            teacher_on_stu_log_probs,
                                            overlap_mask,
                                            response_mask,
                                        )
                                        if figure19_value is not None:
                                            metrics["opd/figure19_metric_schema_version"] = 1
                                            metrics[
                                                "val-extrema/prob_diff_at_max_abs_adv_intersection"
                                            ] = figure19_value.item()

                                        figure19_chunk_size = 1024
                                        for figure19_start in range(0, max_len, figure19_chunk_size):
                                            figure19_end = min(
                                                figure19_start + figure19_chunk_size,
                                                max_len,
                                            )
                                            chunk_value = prob_diff_at_max_abs_advantage(
                                                student_side_advantages[:, figure19_start:figure19_end],
                                                student_log_probs[:, figure19_start:figure19_end],
                                                teacher_on_stu_log_probs[:, figure19_start:figure19_end],
                                                overlap_mask[:, figure19_start:figure19_end],
                                                response_mask[:, figure19_start:figure19_end],
                                            )
                                            if chunk_value is not None:
                                                chunk_key = f"{figure19_start}_{figure19_end}"
                                                metrics[
                                                    "val-extrema/"
                                                    "prob_diff_at_max_abs_adv_intersection_"
                                                    f"chunk_{chunk_key}"
                                                ] = chunk_value.item()
                                    ...
                                    if is_union:
                                        ...
                                        if mask_inter.any():
                                            ...
                                            metrics["val-topk/training_adv_intersection"] = avg_adv_inter.item()
                                        ...
                                        for start_idx in range(0, max_len, chunk_size):
                                            ...
                                            if mask_inter_c.any():
                                                metrics[f"val-topk/training_adv_intersection_chunk_{chunk_key}"] = chunk_student_adv[mask_inter_c].mean().item()
                                        ...
                                        if strategy == "only_tch" and "teacher_in_student_mask" in batch.batch:
                                            ...
                                            if global_mask_inter.any():
                                                ...
                                                metrics["val-topk/training_adv_intersection"] = global_avg_adv_inter.item()
                                            ...
                                            for start_idx in range(0, max_len, chunk_size):
                                                ...
                                                if mask_inter.any():
                                                    metrics[f"val-topk/training_adv_intersection_chunk_{chunk_key}"] = chunk_adv[mask_inter].mean().item()
                                            ...
                                            for start_idx in range(0, max_len, chunk_size):
                                                ...
                                                if mask_inter.any():
                                                    ...
                                                    metrics[f"val-topk/training_adv_intersection_chunk_{chunk_key}"] = avg_adv_inter.item()
